 ...
 * and Pavel Dobryakov's WebGL Fluid Simulation (MIT License)
 *
 * Original: https://github.com/PavelDoGreat/WebGL-Fluid-Simulation
 * License: MIT
 */

// Base vertex shader for full-screen quads
...
// UV Gradient Test Shader (for debugging)
export const uvGradientShader = `
precision mediump float;

uniform vec2 uResolution;

void main () {
    // Use gl_FragCoord for guaranteed visible gradient
    vec2 uv = gl_FragCoord.xy / uResolution;
    // Red = horizontal (0 to 1 left to right)
    // Green = vertical (0 to 1 bottom to top)
    vec3 color = vec3(uv.x, uv.y, 0.0);
    gl_FragColor = vec4(color, 1.0);
}
`;

// Clear shader (fill with color)
...
// Splat shader (add velocity/dye at point)
...
// Vorticity shader (apply curl force)
...
// Display shader (final output with optional bloom)
...
void main () {
    ...
    #ifdef SHADING
        vec3 lc = texture2D(uTexture, vUv - vec2(texelSize.x, 0.0)).rgb;
        vec3 rc = texture2D(uTexture, vUv + vec2(texelSize.x, 0.0)).rgb;
        vec3 tc = texture2D(uTexture, vUv + vec2(0.0, texelSize.y)).rgb;
        vec3 bc = texture2D(uTexture, vUv - vec2(0.0, texelSize.y)).rgb;
